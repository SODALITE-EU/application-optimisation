...
if [ -f "$recipe" ]; then
    ...
    imagefile="${recipe%.*}.imgdir"
    currdir=`pwd`
    recipedir=`dirname ${recipe}`
    recipename=`basename ${recipe}`

    echo "Creating $imagefile using $recipe..."
    cd ${recipedir} && singularity build --sandbox ${currdir}/${imagefile} $recipename && cd ${currdir}
	...
	# Delete image /dev directory, which cannot be exported
	rm -rf ${imagefile}/dev
	# Recreate /dev for mounting host directory
	mkdir ${imagefile}/dev
