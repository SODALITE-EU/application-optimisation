            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'modak_ssh_key', keyFileVariable: 'modak_ssh_key_file', usernameVariable: 'modak_ssh_username')]) {
                    sh """#!/bin/bash
                        ...
                        sleep 100
                        ssh -i ${modak_ssh_key_file} -o StrictHostKeyChecking=no ${modak_ssh_username}@192.168.2.155 "docker exec \\\$(docker ps | grep modak | grep restapi | awk '{print \\\$1}' ) ls -l ../test/integration/hpc.sh"
                       """
