            ...
            steps {
                withCredentials([sshUserPrivateKey(credentialsId: 'modak_ssh_key', keyFileVariable: 'modak_ssh_key_file', usernameVariable: 'modak_ssh_username')]) {
                    sh """#!/bin/bash
                        ...
                        sleep 300
                        ssh -i ${modak_ssh_key_file} -o StrictHostKeyChecking=no ${modak_ssh_username}@192.168.2.155 "docker ps; echo docker exec \$(docker ps | grep restapi | awk '{print \$1}' ) ../test/integration/hpc.sh"
                       """
